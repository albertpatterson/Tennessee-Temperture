...
def getLocTimeTempData(fileName):
    file = open(fileName, "r")
    rawData = json.load(file)
    ...
    time = list(map(lambda datum: createDate(datum["time"]), hourlyData))
    temp_f = list(map(lambda datum: datum["temp_f"], hourlyData))

    return (lat, lon, time, temp_f)
